...
import { storageService } from './services/storageService'; // CORRECTION ICI (était ../services/storageService)
...
const App: React.FC = () => {
  ...
  const initializeSession = async (userProfile: UserProfile, prefs: UserPreferences) => {
    ...
    try {
      ...
      if (history.length === 0) {
        const greeting = (prefs as any).explanationLanguage === 'Français 🇫🇷' ? INITIAL_GREETING_FR : INITIAL_GREETING_MG;
  ...
  const handleUpdateChallengeProgress = (type: 'message_count' | 'lesson_complete' | 'vocabulary') => {
      ...
      if (updated) {
          const newUser = { ...user, dailyChallenges: updatedChallenges, stats: { ...user.stats, xp: user.stats.xp + xpGained } };
          setUser(newUser);
  ...
  const handleFontSizeChange = (size: 'small' | 'normal' | 'large' | 'xl') => {
      if (!user) return;
      const updatedUser = { ...user, preferences: { ...(user.preferences || {}), fontSize: size } as UserPreferences };
      setUser(updatedUser);
      storageService.saveUserProfile(updatedUser);
  };
