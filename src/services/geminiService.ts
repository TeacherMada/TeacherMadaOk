...
const getAvailableKeys = (): string[] => {
    ...
    return Array.from(new Set(keys)).filter((k): k is string => typeof k === 'string' && k.trim().length > 0);
};
...
export const generateVoiceChatResponse = async (
    ...
    return executeWithRetry(async (modelName) => {
        ...
        const systemInstruction = `
            ...
            RULES: KEEP IT SHORT. Max 2 sentences. No lists. No markdown.
        `;
...
export const analyzeVoiceCallPerformance = async (
    ...
    return executeWithRetry(async (modelName) => {
        ...
        const prompt = `
            ...
            Output valid JSON only: { "score": number, "feedback": "string", "tip": "string" }
        `;
...
export const generatePracticalExercises = async (
  ...
  return executeWithRetry(async (modelName) => {
      ...
      const prompt = `
        ...
        Contexte récent: ${recentTopics.substring(0, 500)}.
        Retourne un tableau JSON pur:
        [{
            "type": "multiple_choice" | "true_false" | "fill_blank",
            "question": "string",
            "options": ["string"] (si QCM),
...
export const generateRoleplayResponse = async (
    ...
    return executeWithRetry(async (modelName) => {
        ...
        let prompt = `
            ROLE: Roleplay partner. SCENARIO: ${scenario}.
            LANGUAGE: ${userProfile.preferences?.targetLanguage}. LEVEL: ${userProfile.preferences?.level}.
            INPUT: ${context}
            RESPONSE JSON: { "aiReply": "string", "correction": "string | null" }
        `;

        if (isClosing) {
            prompt = `
                ROLE: Examiner. End scenario: ${scenario}.
                CONVERSATION: ${context}
                RESPONSE JSON: { "aiReply": "Bye", "score": number, "feedback": "string" }
            `;
        }
