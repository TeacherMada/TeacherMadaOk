...
import { X, Smartphone, ShieldCheck, Calculator, ArrowRight, Send, CheckCircle, Copy, Check, Coins, CreditCard, ChevronRight, User, Hash, FileText, Zap, Ticket, Loader2 } from 'lucide-react';
...
const PaymentModal: React.FC<PaymentModalProps> = ({ onClose, user }) => {
  // Global Mode: 'money' or 'coupon'
  const [mode, setMode] = useState<'money' | 'coupon'>('money');
  ...
  const [selectedOperator, setSelectedOperator] = useState<'mvola' | 'airtel' | 'orange' | null>('mvola'); // Default MVola
  ...
  const handleRedeemCoupon = async () => {
      ...
      try {
          ...
          if (result.success) {
              ...
              setCouponFeedback(result.message || "Code invalide.");
          }
      } catch (e) {
          setCouponStatus('error');
          setCouponFeedback("Erreur de connexion.");
      }
  };
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xl animate-fade-in">
      <div className="bg-white dark:bg-[#0B0F19] w-full max-w-md rounded-[2rem] overflow-hidden shadow-2xl border border-white/20 dark:border-slate-800 max-h-[90vh] flex flex-col">
        ...
        {/* Header with Tabs */}
        <div className="relative bg-gradient-to-br from-indigo-600 to-violet-700 shrink-0">
            ...
            <div className="pt-8 px-6 pb-0 text-center z-10 relative">
                <h2 className="text-xl font-black text-white tracking-tight mb-4">Portefeuille</h2>
                <div className="flex bg-black/20 p-1 rounded-xl backdrop-blur-md">
                    <button 
                        onClick={() => setMode('money')}
                        className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all flex items-center justify-center gap-2 ${mode === 'money' ? 'bg-white text-indigo-700 shadow-md' : 'text-indigo-200 hover:bg-white/10'}`}
                    >
                        <Smartphone className="w-4 h-4"/> Mobile Money
                    </button>
                    <button 
                        onClick={() => setMode('coupon')}
                        className={`flex-1 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all flex items-center justify-center gap-2 ${mode === 'coupon' ? 'bg-white text-indigo-700 shadow-md' : 'text-indigo-200 hover:bg-white/10'}`}
                    >
                        <Ticket className="w-4 h-4"/> Code Coupon
                    </button>
                </div>
            </div>
            {/* Curver Bottom */}
            <div className="h-6 bg-white dark:bg-[#0B0F19] rounded-t-[2rem] mt-4 relative z-10 scale-105"></div>
        </div>

        {/* Content Body */}
        <div className="flex-1 overflow-y-auto scrollbar-hide px-6 pb-6 -mt-2">
                    ...
                    {view === 'amount' && (
                        <div className="space-y-6 animate-slide-up">
                            <div className="text-center">
                                <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full text-xs font-bold text-slate-500 mb-2">
                                    <Coins className="w-3 h-3 text-amber-500"/> Solde : {user.credits} CRD
                                </div>
                                <p className="text-slate-800 dark:text-white text-lg font-bold">Choisir un montant</p>
                            </div>
                    ...
                    {view === 'operator' && (
                        <div className="space-y-6 animate-slide-in-right">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Opérateur</span>
                            </div>
                            ...
                            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-3 rounded-xl border border-indigo-100 dark:border-indigo-800 text-center">
                                <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-1">Nom du bénéficiaire</p>
                            ...
                            <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-2xl relative overflow-hidden group cursor-pointer" onClick={() => copyToClipboard(motifCode, 'motif')}>
                                ...
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Motif à indiquer (Important)</p>
                                <div className="flex justify-between items-center">
                                    <code className="font-mono font-bold text-lg text-indigo-600 dark:text-indigo-400">{motifCode}</code>
                                    {copied === 'motif' ? <Check className="w-5 h-5 text-emerald-500"/> : <Copy className="w-5 h-5 text-slate-300 hover:text-indigo-500"/>}
                                </div>
                            </div>
                        ...
                        <div className="space-y-6 animate-slide-in-right h-full flex flex-col justify-between">
                            {!isSent ? (
                                <>
                                    <div className="space-y-4">
                                        ...
                                        <div className="text-center">
                                            ...
                                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
                                                Pour valider vos <strong>{credits} CRD</strong>, copiez la référence du SMS de transaction.
                                            </p>
                                        </div>
                                    ...
                                    >
                                        {loading ? 'Vérification...' : 'Valider ma demande'} <Send className="w-5 h-5" />
                                    </button>
                                </>
                            ) : (
                                <div className="flex flex-col items-center justify-center h-full text-center py-10">
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">L'admin vérifiera votre paiement et créditera votre compte rapidement.</p>
                                </div>
                            )}
                        </div>
                    )}
            ...
            {/* --- MODE 2: COUPON CODE --- */}
            {mode === 'coupon' && (
                <div className="animate-fade-in h-full flex flex-col justify-center pb-10">
                    
                    {couponStatus === 'success' ? (
                        <div className="text-center">
                            <div className="w-24 h-24 bg-gradient-to-tr from-yellow-400 to-orange-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl shadow-orange-500/30 animate-bounce">
                                <Zap className="w-12 h-12 text-white fill-white" />
                            </div>
                            <h3 className="text-3xl font-black text-slate-800 dark:text-white mb-2">Succès !</h3>
                            <p className="text-slate-600 dark:text-slate-300 font-bold text-lg">+{redeemedAmount} Crédits ajoutés</p>
                            <p className="text-xs text-indigo-500 font-bold mt-6 animate-pulse">Mise à jour du compte...</p>
                        </div>
                    ) : (
                        <div className="space-y-8">
                            <div className="text-center">
                                <div className="inline-block p-4 bg-indigo-50 dark:bg-slate-800 rounded-2xl mb-4">
                                    <Ticket className="w-8 h-8 text-indigo-500" />
                                </div>
                                <h3 className="text-xl font-bold text-slate-800 dark:text-white">Avez-vous un code ?</h3>
                                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Entrez le code promo ou le code reçu pour obtenir vos crédits instantanément.</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="relative">
                                    <input 
                                        type="text" 
                                        placeholder="CODE-PROMO-123" 
                                        value={couponCode}
                                        onChange={(e) => { setCouponCode(e.target.value.toUpperCase()); setCouponStatus('idle'); }}
                                        className={`w-full text-center text-2xl font-black tracking-widest p-5 rounded-2xl bg-slate-50 dark:bg-slate-800 border-2 outline-none uppercase placeholder:text-slate-300 dark:placeholder:text-slate-700 transition-all ${couponStatus === 'error' ? 'border-red-500 text-red-500' : 'border-slate-200 dark:border-slate-700 focus:border-indigo-500 text-slate-800 dark:text-white'}`}
                                ...
                                {couponStatus === 'error' && (
                                    <div className="text-center text-red-500 font-bold text-sm bg-red-50 dark:bg-red-900/10 p-2 rounded-xl animate-shake">
                                        {couponFeedback}
                                    </div>
                                )}
                                ...
                                <button 
                                    ...
                                    className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg hover:shadow-indigo-500/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Valider le code
                                </button>
                            </div>
