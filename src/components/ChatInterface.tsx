  ...
  const handleSpeak = async (text: string) => {
    ...
    try {
        ...
        if (rawPCM) {
            ...
            const int16 = new Int16Array(rawPCM.buffer);
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      if (isCallActive) {
          ...
          finalReply = await sendMessageToGeminiStream(textToSend, user.id, updatedHistory, (txt) => setStreamingText(txt));
      }
      
      const aiMsg: ChatMessage = { id: (Date.now() + 1).toString(), role: 'model', text: finalReply, timestamp: Date.now() };
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-white/90 dark:bg-slate-900/95 backdrop-blur-md shadow-sm h-14 px-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowSmartOptions(!showSmartOptions)} className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-800 dark:text-indigo-300 rounded-full border border-indigo-100 dark:border-indigo-900/50">
             ...
             <span className="text-[10px] font-black uppercase">{preferences.targetLanguage.split(' ')[0]}</span>
      ...
      <div id="chat-feed" className="flex-1 overflow-y-auto p-3 md:p-6 space-y-5 pt-16 pb-4 scrollbar-hide">
        ...
        {streamingText && (
             <div className="flex justify-start animate-fade-in">
                 ...
                 <div className="bg-white dark:bg-slate-800 px-4 py-3 rounded-[1.3rem] rounded-tl-none border border-slate-50 shadow-sm"><MarkdownRenderer content={streamingText} /></div>
             </div>
        )}
        {isLoading && !streamingText && (
             <div className="flex justify-start animate-fade-in"><div className="bg-white dark:bg-slate-800 px-4 py-3 rounded-2xl border border-slate-50 shadow-sm"><Loader2 className="w-4 h-4 animate-spin text-indigo-500"/></div></div>
        )}
