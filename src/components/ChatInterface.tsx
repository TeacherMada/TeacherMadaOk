...
// Animation fluide de 3 points pour simuler la réflexion/écriture
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  // --- LESSON NUMBER LOGIC ---
  // 1. Scan chat history to find the LAST lesson generated by AI.
  const lastLessonInChat = useMemo(() => {
    ...
    return 0; // No lesson found in current chat session
  }, [messages]);

  // 2. Logic for "Header Display": Show current lesson active in chat, or fallback to stored progress + 1
  ...
  // 3. Logic for "Next Button": Always target the lesson AFTER the one currently in chat.
  // If no lesson in chat, target stored progress + 1.
  const nextTargetLesson = useMemo(() => {
      if (lastLessonInChat > 0) return lastLessonInChat + 1;
      return progressData.lessonsDone + 1;
  }, [lastLessonInChat, progressData.lessonsDone]);
  ...
  }, [messages, isTrainingMode, isDialogueActive, isLoading]); // Added isLoading so it scrolls when typing bubble appears
        ...
        <div className="max-w-4xl mx-auto mb-2 flex items-center gap-2 px-2 overflow-x-auto scrollbar-hide">
            ...
            <div className="flex-1 mx-3 flex flex-col justify-center">
             ...
             <Tooltip text="Leçon Suivante">
                <button 
                    onClick={() => handleSend(`Génère la suite : LEÇON ${nextTargetLesson}`)} 
                    disabled={isSending} 
                ...
                >
                    {isSending && input.includes("Génère la suite") ? "Génération..." : "Suivant"} <ArrowRight className="w-3 h-3" />
                </button>
            </Tooltip>
        </div>
