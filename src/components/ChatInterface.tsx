...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  const handleExportImage = async (msgId: string) => {
      ...
      try {
          ...
          const canvas = await (window as any).html2canvas(element, { scale: 2, backgroundColor: null, useCORS: true });
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
                            ...
                            <div className="p-2 border-b border-slate-100 dark:border-slate-800 mb-2">
                                <div className="relative flex items-center">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-slate-50 dark:bg-black/20 text-sm py-2 pl-9 pr-10 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                                    />
      ...
      <div id="chat-feed" className="flex-1 overflow-y-auto p-3 md:p-4 space-y-4 pt-20 pb-4 scrollbar-hide">
            ...
            return (
                <div key={msg.id} id={`msg-${msg.id}`} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                    <div className={`flex max-w-[90%] md:max-w-[80%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                        ...
                        <div className={`px-4 py-3 rounded-2xl shadow-sm ${textSizeClass} ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 dark:text-slate-200 text-slate-800 rounded-tl-none border border-slate-100 dark:border-slate-700'} ${isCurrentMatch ? 'ring-4 ring-yellow-400/50 shadow-yellow-200 dark:shadow-none' : isMatch ? 'ring-2 ring-yellow-200/50' : ''}`}>
                                ...
                                <>
                                    ...
                                    {index === 0 && messages.length === 1 && (
                                        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-center">
                                            <button onClick={() => handleSend("Commence le cours / Leçon 1")} className="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-bold rounded-full shadow-lg flex items-center gap-2 hover:scale-105 transition-transform">
                                                <span>COMMENCER</span> <ArrowRight className="w-4 h-4" />
                                            </button>
                                        </div>
                                    )}
                                    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-100 dark:border-slate-700/50" id={`msg-content-${msg.id}`}>
                                        ...
                                        <button onClick={() => { navigator.clipboard.writeText(msg.text); setCopiedId(msg.id); setTimeout(() => setCopiedId(null), 2000); }} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors">{copiedId === msg.id ? <Check className="w-4 h-4 text-emerald-500"/> : <Copy className="w-4 h-4 text-slate-400 hover:text-indigo-500"/>}</button>
                                        <button onClick={() => handleExportPDF(msg.text)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors"><FileText className="w-4 h-4 text-slate-400 hover:text-red-500"/></button>
        ...
        {(isLoading || isAnalyzing) && (
             <div className="flex justify-start animate-fade-in">
                 ...
                 <div className="bg-white dark:bg-slate-800 px-4 py-3 rounded-2xl rounded-tl-none border shadow-sm flex items-center gap-2 ml-2">
      ...
      <div id="input-area" className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-t border-slate-100 dark:border-slate-800 p-3 md:p-4 sticky bottom-0">
        ...
        {/* Quick Actions Bar with Progress & Call Button */}
        <div className="max-w-4xl mx-auto mb-2 flex items-center gap-2 px-2 overflow-x-auto scrollbar-hide">
            <Tooltip text="Appel Vocal">
                <button onClick={initiateCallFlow} className="p-2 bg-purple-50 dark:bg-purple-900/20 rounded-full shadow-sm border border-purple-100 dark:border-purple-800 text-purple-600 dark:text-purple-400 hover:bg-purple-100 transition-colors">
