...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  useEffect(() => {
      ...
      let bars = Array.from({ length: 40 }, () => Math.random() * 20 + 5);
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      storageService.saveChatHistory(user.id, [...newHistory, aiMsg], preferences.targetLanguage);
          ...
          <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col items-center justify-between p-8 text-white animate-fade-in font-sans overflow-hidden">
              ...
              <div className="w-full flex justify-between items-center z-10">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs font-black uppercase tracking-widest text-emerald-400">Appel Chiffré</span>
                    </div>
              ...
              <div className="flex flex-col items-center justify-center w-full z-10 flex-1">
                  <div className="relative mb-8">
                      <div className={`w-32 h-32 rounded-[2.5rem] bg-gradient-to-br from-indigo-500 to-purple-600 p-1 shadow-2xl transition-transform duration-500 ${isPlayingAudio ? 'scale-110 rotate-3' : 'scale-100 rotate-0'}`}>
                          <div className="w-full h-full bg-slate-900 rounded-[2.2rem] flex items-center justify-center overflow-hidden border-2 border-white/10">
                              <img src="https://i.ibb.co/B2XmRwmJ/logo.png" className="w-20 h-20 object-contain" alt="Tutor" />
                          </div>
                      </div>
                      {isListening && <div className="absolute -bottom-2 -right-2 bg-white text-slate-900 p-1.5 rounded-full shadow-lg animate-bounce"><Mic className="w-4 h-4"/></div>}
                  </div>

                  <h3 className="text-2xl font-black mb-2 tracking-tight">TeacherMada</h3>
                  ...
                  <div className="mt-8 px-6 text-center h-16 max-w-sm">
                      ...
                      {!interimTranscript && !isPlayingAudio && !isLoading && <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">Parlez maintenant</p>}
                  </div>
              </div>
              ...
              <div className="w-full max-w-md grid grid-cols-3 gap-4 pb-8 z-10">
                  <button onClick={() => setIsMuted(!isMuted)} className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 uppercase">Silencieux</span>
                  </button>
                  <button onClick={handleEndCall} className="flex flex-col items-center gap-2">
                      <div className="p-7 bg-red-500 rounded-full shadow-[0_15px_30px_-5px_rgba(239,68,68,0.4)] hover:bg-red-600 transition-all transform active:scale-95">
                  ...
                  <button onClick={toggleListening} className="flex flex-col items-center gap-2">
                      <div className={`p-5 rounded-full transition-all ${isListening ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white/5 text-white border border-white/10 hover:bg-white/10'}`}>
                          <Mic className="w-6 h-6"/>
                      </div>
                      <span className="text-[10px] font-bold text-slate-500 uppercase">Micro</span>
                  </button>
              </div>
              ...
              {showVoiceSettings && (
                  <div className="absolute inset-0 bg-slate-950/95 z-50 p-8 flex flex-col animate-slide-up">
                      <div className="flex justify-between items-center mb-8">
                          <h3 className="text-xl font-black">Voix du Tuteur</h3>
                          <button onClick={() => setShowVoiceSettings(false)} className="p-2 bg-white/10 rounded-full"><X className="w-5 h-5"/></button>
                      </div>
                      <div className="space-y-4">
                          {VOICE_OPTIONS.map(v => (
                              <button 
                                ...
                                className={`w-full p-6 rounded-[1.5rem] border-2 text-left transition-all flex justify-between items-center ${user.preferences?.voiceName === v.id ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5 bg-white/5 hover:border-white/20'}`}
                              >
                                  <div>
                                      <div className="font-bold text-lg">{v.label}</div>
                                      <div className="text-xs text-slate-400">{v.desc}</div>
                                  </div>
                                  {user.preferences?.voiceName === v.id && <Check className="w-6 h-6 text-indigo-500"/>}
                              </button>
                          ))}
                      </div>
                  </div>
