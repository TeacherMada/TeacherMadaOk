...
import { X, Trophy, Flame, LogOut, Sun, Moon, BookOpen, CheckCircle, Calendar, Target, Activity, Edit2, Save, Type, Coins, CreditCard, ChevronRight, Check, Shield, Download, Upload, Loader2, Sparkles, Plus, Trash2, Sliders, Volume2, Search, BrainCircuit } from 'lucide-react';
...
const SmartDashboard: React.FC<SmartDashboardProps> = ({ 
  ...
  const handleExtractVocab = async () => {
    ...
    try {
        ...
        const updatedVocab = [...vocabulary, ...newWords];
  ...
  const toggleMastered = (id: string) => {
      const updated = vocabulary.map(v => v.id === id ? { ...v, mastered: !v.mastered } : v);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      <div className="relative w-full md:w-[480px] h-full bg-white dark:bg-[#0F1422] shadow-2xl flex flex-col border-l border-white/10 animate-slide-in-right">
        {/* Header */}
        <div className="bg-white dark:bg-[#131825] p-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
             <div className="flex items-center gap-4">
                 <div className="w-14 h-14 bg-gradient-to-tr from-indigo-500 to-violet-600 rounded-2xl shadow-lg flex items-center justify-center text-xl font-black text-white">{user.username.charAt(0).toUpperCase()}</div>
                 <div>
                     <h2 className="text-lg font-bold text-slate-900 dark:text-white">{user.username}</h2>
                     <p className="text-xs text-slate-500 font-bold uppercase">{user.preferences?.targetLanguage}</p>
                 </div>
             </div>
        ...
        <div className="px-6 py-4">
            <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
                ...
                <button onClick={() => setActiveTab('vocab')} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${activeTab === 'vocab' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500'}`}>Mots</button>
                ...
                <div className="space-y-6">
                    <div className="bg-slate-50 dark:bg-[#1A2030] rounded-3xl p-6 border border-slate-100 dark:border-white/5 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                             ...
                             <h3 className="font-black text-slate-800 dark:text-white">Voix du Prof</h3>
                        </div>
                                ...
                                <div key={word.id} className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${word.mastered ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100' : 'bg-white dark:bg-[#1A2030] border-slate-100 dark:border-white/5'}`}>
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button onClick={() => handlePlayWord(word.word, word.id)} className={`p-1.5 rounded-full ${playingId === word.id ? 'text-indigo-600 animate-pulse' : 'text-slate-400'}`}><Volume2 className="w-4 h-4"/></button>
                                        </div>
